...
const PackageCard = ({ image, title, subtitle, description, features, index, includedTitle, cta }) => {
  return (
    <motion.div 
    ...
    >
      <div className="flex flex-col lg:flex-row">
        <motion.div className="w-full lg:w-2/5 relative overflow-hidden" whileHover={{ scale: 1.03 }} transition={{ duration: 0.4 }}>
          <img src={image} alt={title} className="w-full h-48 sm:h-56 md:h-64 lg:h-full object-cover transform transition-transform duration-500 hover:scale-105" />
        </motion.div>
        
        <div className="w-full lg:w-4/5 p-4 sm:p-5 md:p-6 bg-gradient-to-br from-gray-50 to-white">
          <motion.div className="mb-3 sm:mb-4" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: index * 0.2 + 0.1 }} viewport={{ once: true }}>
            ...
            <div className="space-y-1">
              ...
              {description && <p className="text-xs sm:text-sm text-gray-500 italic">{description}</p>}
            </div>
          </motion.div>
          
          <motion.div className="mb-4 sm:mb-5" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: index * 0.2 + 0.2 }} viewport={{ once: true }}>
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.2 + 0.4 }} viewport={{ once: true }}>
